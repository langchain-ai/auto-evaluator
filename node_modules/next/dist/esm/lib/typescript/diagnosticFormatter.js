...
function getFormattedLinkDiagnosticMessageText(diagnostic) {
    ...
    if (typeof message === "string" && diagnostic.code === 2322) {
        ...
        if (match) {
            ...
            var ref, ref1;
            const relatedMessage = (ref = diagnostic.relatedInformation) == null ? void 0 : (ref1 = ref[0]) == null ? void 0 : ref1.messageText;
    ...
    if (typeof messageText === "string") {
        ...
        switch(message.code){
                        ...
                        for (const item of next){
                            switch(item.code){
                                case 2322:
                                    ...
                                    if (types) {
                                        main += "\n" + " ".repeat(indent * 2);
                                        main += `Expected "${chalk.bold(types[2])}", got "${chalk.bold(types[1])}".`;
                                    }
                                    break;
                                default:
                            }
            ...
            case 2559:
                ...
                if (invalid1) {
                    const main = `${type} "${chalk.bold(relativeSourceFilepath)}" contains an invalid type "${chalk.bold(invalid1[1])}" as ${invalid1[2]}.`;
                    return main;
                }
                break;
            default:
