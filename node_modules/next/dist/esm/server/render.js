...
export async function renderToHTML(req, res, pathname, query, renderOpts) {
    ...
    const { err , dev =false , ampPath ="" , pageConfig ={} , buildManifest , reactLoadableManifest , ErrorDebug , getStaticProps , getStaticPaths , getServerSideProps , isDataReq , params , previewProps , basePath , devOnlyCacheBusterQueryString , images , runtime: globalRuntime , App ,  } = renderOpts;
    ...
    if (getServerSideProps && !isFallback) {
        ...
        try {
            data = await getTracer().trace(RenderSpan.getServerSideProps, async ()=>getServerSideProps({
                    req: req,
                    res: resOrProxy,
                    query,
    ...
    const documentResult = await getTracer().trace(RenderSpan.renderDocument, async ()=>renderDocument());
    if (!documentResult) {
        return null;
    }
