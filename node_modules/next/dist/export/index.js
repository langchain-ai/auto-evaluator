...
async function exportApp(dir, options, span, configuration) {
  ...
  Read more: https://nextjs.org/docs/messages/export-image-api`);
        ...
        if (options.buildExport && hasAppDir) {
            ...
            renderOpts.serverComponentManifest = require((0, _path).join(distDir, _constants1.SERVER_DIRECTORY, _constants1.CLIENT_REFERENCE_MANIFEST + ".json"));
            // @ts-expect-error untyped
       ...
       */ if (!exportPathMap["/404.html"]) {
            ...
            return pageExportSpan.traceAsyncFn(async ()=>{
                ...
                if (options.buildExport && configuration) {
                    ...
                    if (typeof result.fromBuildExportMeta !== "undefined") {
                        configuration.initialPageMetaMap[path] = result.fromBuildExportMeta;
                    }
        ...
        if (!options.buildExport && prerenderManifest) {
            await Promise.all(Object.keys(prerenderManifest.routes).map(async (route)=>{
                ...
                const pagePath = (0, _require).getPagePath(pageName, distDir, undefined, false);
