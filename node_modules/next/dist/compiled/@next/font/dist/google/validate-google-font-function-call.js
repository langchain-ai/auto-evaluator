 ...
 */
function validateGoogleFontFunctionCall(functionName, fontFunctionArgument, config) {
    var _a;
    let { weight, style, preload = true, display = 'swap', axes, fallback, adjustFontFallback = true, variable, subsets: callSubsets, } = fontFunctionArgument || {};
    ...
    if (availableSubsets.length === 0) {
        ...
        if (preload && !callSubsets && !(config === null || config === void 0 ? void 0 : config.subsets)) {
            (0, next_font_error_1.nextFontError)(`Missing selected subsets for font \`${fontFamily}\`. Please specify subsets in the function call or in your \`next.config.js\`. Read more: https://nextjs.org/docs/messages/google-fonts-missing-subsets`);
        }
